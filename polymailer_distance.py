...
def choose_cut_side(poly_mask, product, intrinsics, depth_mm):
    ...
    result["cut_row_full"] = int(ROI_Y1 + cut_row_roi)
...
def run_sam2_polymailer(frame_bgr, depth_aligned, mask_generator, intrinsics):
    ...
    if points_full is not None and len(points_full) == 4:
        ...
        cx_full = ROI_X1 + int(bx + bw / 2)
        top_edge_full = (cx_full, ROI_Y1 + int(by))
        bottom_edge_full = (cx_full, ROI_Y1 + int(by + bh - 1))

    top_edge_roi = (top_edge_full[0] - ROI_X1, top_edge_full[1] - ROI_Y1)
    bottom_edge_roi = (bottom_edge_full[0] - ROI_X1, bottom_edge_full[1] - ROI_Y1)

    top_edge_depth, _ = center_depth_mm(depth_roi, poly["mask"], top_edge_roi)
    bottom_edge_depth, _ = center_depth_mm(depth_roi, poly["mask"], bottom_edge_roi)

    if top_edge_depth is None:
        top_edge_depth = polymailer_face_depth_mm
    if bottom_edge_depth is None:
        bottom_edge_depth = polymailer_face_depth_mm
    ...
    print(f"  cut_side:                          {cut['cut_side']}")
    print(f"  top_gap_mm:                        {fmt3(cut['top_gap_mm'])}")
    print(f"  bottom_gap_mm:                     {fmt3(cut['bottom_gap_mm'])}")
    print(f"  top_edge_x_mm:                     {fmt3(edges['top_x_mm'])}")
    print(f"  top_edge_y_mm:                     {fmt3(edges['top_y_mm'])}")
    print(f"  top_edge_z_mm:                     {fmt3(edges['top_z_mm'])}")
    print(f"  bottom_edge_x_mm:                  {fmt3(edges['bottom_x_mm'])}")
    print(f"  bottom_edge_y_mm:                  {fmt3(edges['bottom_y_mm'])}")
    print(f"  bottom_edge_z_mm:                  {fmt3(edges['bottom_z_mm'])}")
...
def main():
    ...
    with pipeline:
        ...
        while pipeline.isRunning():
            ...
            # Mono sensors (OV7251) are 640x480; upscale depth to the RGB
            # resolution so the ROI, intrinsics, and previews all line up.
            ...
            if key == ord(" "):
                ...
                json_result = {
                    ...
                    "cut_side": result["cut"]["cut_side"],
                    "top_gap_mm": json_number(result["cut"]["top_gap_mm"]),
                    "bottom_gap_mm": json_number(result["cut"]["bottom_gap_mm"]),
                    "top_edge_x_mm": json_number(result["edges"]["top_x_mm"]),
                    "top_edge_y_mm": json_number(result["edges"]["top_y_mm"]),
                    "top_edge_z_mm": json_number(result["edges"]["top_z_mm"]),
                    "bottom_edge_x_mm": json_number(result["edges"]["bottom_x_mm"]),
                    "bottom_edge_y_mm": json_number(result["edges"]["bottom_y_mm"]),
                    "bottom_edge_z_mm": json_number(result["edges"]["bottom_z_mm"]),
                }
